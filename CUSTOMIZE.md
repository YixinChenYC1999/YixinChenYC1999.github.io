...
├── 📂 _bibliography/
│   └── 📄 publications.bib: bibliography in BibTeX format
...
To add publications create a new entry in the [\_bibliography/publications.bib](_bibliography/publications.bib) file. You can find the BibTeX entry of a publication in Google Scholar by clicking on the quotation marks below the publication title, then clicking on "BibTeX", or also in the conference page itself. By default, the publications will be sorted by year and the most recent will be displayed first. You can change this behavior and more in the `Jekyll Scholar` section in [\_config.yml](_config.yml) file.
